...
**四种检索模式**
1. **自动浮现**（`breath()` 无参数）：按衰减分排序推送，钉选桶始终展示；**冷启动**——从未被访问（`activation_count==0`）且 `importance≥8` 的新桶优先插最前（最多 2 个），让新建的重要记忆不被衰减分埋没；其余 Top-1 固定 + Top-20 随机打乱（引入多样性），有 token 预算（默认 10000）
