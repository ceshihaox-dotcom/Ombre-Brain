...
2. 同一话题的零散信息整合为一条记忆;若一段长发言里包含多个独立 point/金句/认知, 优先拆成多条 item, 不要硬塞一条。
   ⚠️ 但如果整段内容主题集中(围绕单一事件/感悟/解构), 即使能找出多个 sub-point, 也作为**一条 item** 输出 — content 写完整(接近 400 字上限是 OK 的), 各 sub-point 用自然段连缀, 不要为了凑数硬拆。
   判断标准: 如果拆出的 N 条 item 共享同一标题/主题, 就是凑数, 应该合一条
