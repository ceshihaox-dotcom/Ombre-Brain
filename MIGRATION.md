...
**修复方法**:暂无自动迁移工具(因为不知道老数据是 UTC 还是本地时间),但**新桶的时间显示是对的**。老桶若偏移很影响:显示时间取 `event_time`(没有才回退 `created`)——手动改 `event_time` 即可纠正显示,没有 event_time 的桶则补一个。`last_active` 只影响衰减计时、不影响显示,可不管。
