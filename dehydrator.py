...
# --- 正文重写提示词: 根据原文重新提炼出干净的单条正文(不拆分) ---
# 用于工作台"重新脱水 + 同时重写正文"勾选项
REGEN_CONTENT_PROMPT = """你是一个记忆正文重写专家。下面是一条记忆的**原文**(可能是聊天截选/日记片段/笔记原稿,通常较长且杂乱)。请按这条记忆的主题,把原文重写为**一段干净紧凑的叙述正文**。

重写规则:
1. 输出**单条正文**,不要拆成多条,不要加标题/小节/列表符号
2. 保留所有核心事实、情绪、待办、关键数字日期
3. 去除冗余口水话、客套、重复表述
4. 总长度尽量在原文 30%~60% 之间(原文很短就少压一些)
5. 对人名、地名、专有名词用 [[双链]] 标记(如 [[婷易]]、[[Obsidian]]),普通词汇不要加
6. **称谓统一(强制)**: 用户(原对话发问方)一律称「Rin」或第二人称「你」, AI(原对话回应方)一律称「克」。原文 user/assistant/claude/Claude/AI/ai/小克 等任何变体一律换;引用第三方人物原样保留
7. 直接输出重写后的正文文本,**不要**加 JSON、不要 markdown 代码块、不要前言后记"""
...
class Dehydrator:
    ...
    # ---------------------------------------------------------
    # Regenerate content body from raw source
    # 根据原文重写正文(用于"重新脱水 + 同时重写正文"勾选)
    # 单独走一遍 LLM, 输出纯文本正文; 不做 metadata 提炼(由调用方再串 redehydrate)
    # ---------------------------------------------------------
    async def regenerate_content_from_source(self, source: str) -> dict:
        """
        以 source(metadata.raw_source 或类似原文)为输入, 让 LLM 重写出干净的单条正文.
